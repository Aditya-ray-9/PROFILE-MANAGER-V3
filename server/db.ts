...
async function initDatabase() {
  ...
  const databaseUrl = process.env.NEON_DATABASE_URL || process.env.DATABASE_URL || "postgresql://neondb_owner:npg_XP8xBDvj9gIZ@ep-noisy-poetry-a4yk8qnu-pooler.us-east-1.aws.neon.tech/neondb?sslmode=require";
  ...
  if (databaseUrl) {
    try {
      ...
      console.log("Database connection will use PostgreSQL");
    } catch (e) {
      console.error("Error parsing DATABASE_URL:", e);
    }
  } else {
    console.error("No database URL found in environment variables. Please set NEON_DATABASE_URL or DATABASE_URL.");
  }
